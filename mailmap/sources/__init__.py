...
def select_source(config: Config) -> EmailSource:
    """Select the best email source based on configuration.

    Selection logic:
    1. Prefer Thunderbird cache (fast, local, no network)
    2. Fall back to IMAP if Thunderbird not available
    ...
    Raises:
        ValueError: If no email source is available
    """
    # Try Thunderbird first (fast local access)
    profile_path = None
